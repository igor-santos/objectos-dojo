...
especificação, precisariamos buscar um aluno na tabela `ALUNO` desta forma.
	  ...
	  public void ordenacao_padrao() {
	    FakeRequestWrapper wrapper = new FakeRequestWrapper();
	    Aluno aluno = buscarAluno.porMatricula(20121000);
	    
	    List<ConsultaDeLivroDTO> res = consulta.list(wrapper, aluno.getMatricula());
	  } 
	  
Passando apenas a matrícula e não o objeto __aluno__, evitamos de realizar um `join` na _query_ da
consulta. A questão é, por que juntar duas tabelas inteiras (com _join_) se podemos obter o mesmo
resultado em um `where` onde os atributos são equivalentes nas duas tabelas? 

A consulta torna-se menor e mais eficiente (neste caso).

Há situações onde você precise passar o objeto no argumento e realizar um `join` diferente do item
acima. Em outras palavras, tudo dependerá dos requisitos.
